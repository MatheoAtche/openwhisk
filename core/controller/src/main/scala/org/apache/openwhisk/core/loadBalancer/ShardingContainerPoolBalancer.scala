...
//    }
    val chosen: Option[InvokerInstanceId] = if (invokersToUse.nonEmpty) {
      Option(invokersToUse(0).id)  // shceduling performed by ad-hoc entity
    } else {
      None
    }
...
//      see@ https://stackoverflow.com/questions/37523542/add-field-to-existing-json-object-with-sprayjson-scala
        ...
        val scheduler: JsObject = JsObject(
          "controllerEnd" -> System.currentTimeMillis().toJson,
          "kind" -> action.exec.kind.toJson,
          "target" -> targetInvoker.toJson,
          "limits" -> JsObject(
            "memory" -> action.limits.memory.megabytes.toJson,
            "concurrency" -> action.limits.concurrency.toJson,
            "timeout" -> action.limits.timeout.millis.toJson,
            "userMemory" -> invoker.userMemory.toMB.toJson
          )
        )
        val newContent: Option[JsObject] = msg.content match {
          case Some(value) => if (value.asJsObject.fields.contains("$scheduler")) {
            // $scheduler fields from msg.content will be overwritten by those generated by controller
            val mergedScheduler: JsObject = JsObject(value.asJsObject.fields.get("$scheduler").toJson.asJsObject.fields ++ scheduler.fields)
            Some(JsObject(value.asJsObject.fields + ("$scheduler" -> mergedScheduler)))
          } else {
        ...
        val activationResult = setupActivation(newMsg, action, invoker)
        sendActivationToInvoker(messageProducer, newMsg, invoker).map(_ => activationResult)
      }
      .getOrElse {
